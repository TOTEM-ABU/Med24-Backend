...
export class MedicationsService {
	...
	async create(createMedicationDto: CreateMedicationDto) {
		try {
			...
			throw new HttpException('Medication create failed!', HttpStatus.BAD_REQUEST);
		}
	}
	...
	async findOne(id: string) {
		try {
			...
			if (error instanceof HttpException) throw error;
			throw new HttpException('Medication fetch one failed!', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}

	async update(id: string, updateMedicationDto: UpdateMedicationDto) {
		const existing = await this.prisma.medications.findUnique({ where: { id } });
		if (!existing) {
			throw new HttpException('Medication not found!', HttpStatus.NOT_FOUND);
		}
		try {
			...
			if (error instanceof HttpException) throw error;
			throw new HttpException('Medication update failed!', HttpStatus.BAD_REQUEST);
		}
	}
	...
	async remove(id: string) {
		...
		try {
			...
			if (error instanceof HttpException) throw error;
			throw new HttpException('Medication delete failed!', HttpStatus.BAD_REQUEST);
		}
	}
}

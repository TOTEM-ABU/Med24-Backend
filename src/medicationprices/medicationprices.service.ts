...
export class MedicationpricesService {
	...
	async create(createMedicationpriceDto: CreateMedicationpriceDto) {
		...
		try {
			...
			await Promise.all([
				this.prisma.medications.findUniqueOrThrow({ where: { id: medicationsId } }),
				this.prisma.pharmacies.findUniqueOrThrow({ where: { id: pharmaciesId } }),
			]);
			...
			throw new HttpException('Medication price create failed!', HttpStatus.BAD_REQUEST);
		}
	}

	async findAll(params: {
		search?: string; // searches by medication or pharmacy name
		sort?: 'asc' | 'desc';
		...
		if (search) {
			where.OR = [
				{ Medications: { name: { contains: search, mode: 'insensitive' as const } } },
				{ Pharmacies: { name: { contains: search, mode: 'insensitive' as const } } },
			];
		}
	...
	async findOne(id: string) {
		try {
			...
			if (!price) {
				throw new HttpException('Medication price not found!', HttpStatus.NOT_FOUND);
			}
			return price;
		} catch (error) {
			if (error instanceof HttpException) throw error;
			throw new HttpException('Medication price fetch one failed!', HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}

	async update(id: string, updateMedicationpriceDto: UpdateMedicationpriceDto) {
		const existing = await this.prisma.medicationPrices.findUnique({ where: { id } });
		if (!existing) {
			throw new HttpException('Medication price not found!', HttpStatus.NOT_FOUND);
		}
		try {
			...
			if (error instanceof HttpException) throw error;
			throw new HttpException('Medication price update failed!', HttpStatus.BAD_REQUEST);
		}
	}
	...
	async remove(id: string) {
		...
		try {
			...
			if (error instanceof HttpException) throw error;
			throw new HttpException('Medication price delete failed!', HttpStatus.BAD_REQUEST);
		}
	}
}
